...
export default function DailyJournalFeed({ posts = [] }: { posts?: any[] }) {
  ...
  const fallbackHero = "/images/nature/ocean_zen_blue_gradient.jpg";
  ...
  const selectedPostImages = useMemo(() => {
    if (isReadingPrologue) return ["/images/nature/ocean_zen_blue_gradient.jpg"];
  ...
  const flipboardCards = useMemo(() => {
    ...
    const prologueCard = {
      ...
      img: "/images/nature/ocean_zen_blue_gradient.jpg",
      post: null,
      isPrologue: true,
      postIndex: -1,
    };
    ...
    <>
      <style>{`
        ...
        *::-webkit-scrollbar {
        ...
        .pj-left-content {
          ...
          padding: 3.5rem clamp(1.5rem, 5vw, 4rem) calc(env(safe-area-inset-bottom, 24px) + 46px);
          box-sizing: border-box;
          z-index: 10;
        ...
        @media (max-width: 860px) {
          ...
          .pj-left-content {
            ...
            padding: 3.5rem 1.35rem calc(env(safe-area-inset-bottom, 24px) + 46px) 1.35rem !important;
            box-sizing: border-box !important;
      ...
      <div className={`pj-root${selectedPost || isReadingPrologue ? " has-selected-post" : ""}`} ref={mobileScrollRef}>
        ...
        >
          ...
          {/* Dashed progress indicators: max 6 most recent posts, fixed total width, each dash = equal share */}
          {flipboardCards.length > 1 && (() => {
            const MAX_DASHES = 6;
            const dashCards = flipboardCards.slice(0, MAX_DASHES);
            // heroIndex within the displayed dash range
            const activeDashIdx = heroIndex < MAX_DASHES ? heroIndex : -1;
            return (
              <div
