...
import {
  ...
  Clock, X, ChevronDown, ChevronUp, Repeat, Trash2, Sparkles
} from 'lucide-react';
import { AgendaItem, MessageSummary, MessageDetail } from '@/types/mail';
import { fetchAgendas, saveAgenda, toggleAgendaApi, deleteAgendaApi } from '@/lib/api';
import { AiExecutiveWidget } from './AiExecutiveWidget';
...
interface CalendarAgendaWidgetProps {
...
export const CalendarAgendaWidget: React.FC<CalendarAgendaWidgetProps> = ({
  selectedMessage,
  messages = [],
  onSelectMessage,
  onUseSuggestedReply,
}) => {
  const [activeTab, setActiveTab] = useState<'ai' | 'agenda'>('ai');
  const [isMonthExpanded, setIsMonthExpanded] = useState(false);
  const [currentMonth, setCurrentMonth] = useState(() => new Date());
  const [selectedDate, setSelectedDate] = useState(() => new Date());

  // Auto switch to AI Copilot whenever user opens an email
  useEffect(() => {
    if (selectedMessage) {
      setActiveTab('ai');
    }
  ...
  return (
    <aside className="w-full h-full bg-[var(--card-bg)] border border-[var(--card-border)] rounded-3xl p-3.5 shadow-card flex flex-col min-h-0 overflow-hidden font-sans select-none shrink-0">
      {/* Calendar Header */}
      ...
      {/* ── Segmented Switch: AI Copilot vs Agenda ── */}
      <div className="my-2 p-0.5 rounded-full bg-[var(--bg-secondary)] border border-[var(--card-border)] flex items-center shrink-0">
        <button
          onClick={() => setActiveTab('ai')}
          className={`flex-1 py-1 px-2 rounded-full text-xs font-bold flex items-center justify-center gap-1.5 apple-transition cursor-pointer ${
            activeTab === 'ai'
              ? 'bg-gradient-to-r from-purple-600 to-blue-600 text-white shadow-xs'
              : 'text-[var(--text-muted)] hover:text-[var(--text-primary)]'
          }`}
        >
          <Sparkles className={`w-3 h-3 ${activeTab === 'ai' ? 'text-amber-300' : 'text-purple-500'}`} />
        ...
        >
          ...
          {selectedDayAgendas.length > 0 && (
            <span className={`text-[9px] px-1.5 py-0.2 rounded-full font-extrabold ${
              activeTab === 'agenda' ? 'bg-white/20 text-white' : 'bg-blue-500/15 text-blue-600 dark:text-blue-400'
            }`}>
              {selectedDayAgendas.length}
            </span>
          )}
        </button>
      </div>
      ...
      {activeTab === 'ai' ? (
        ...
        <div className="flex-1 flex flex-col min-h-0 overflow-hidden">
          ...
          {showAddAgenda && (
            <form onSubmit={handleCreateAgenda} className="mb-2 p-2.5 bg-[var(--bg-secondary)] rounded-2xl border border-[var(--card-border)] space-y-2 animate-toast shrink-0">
