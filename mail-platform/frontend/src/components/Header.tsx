    ...
    <>
      ...
      {/* ──────────────────────────────────────────────────────────────────────────
          3. DESKTOP HEADER (Always shown on Desktop screen >= md)
             - Left: Clean Folder Name (Inbox)
             - Right: Search Input (above reading pane) + Refresh + Settings in top right corner
          ────────────────────────────────────────────────────────────────────────── */}
      <header className="hidden md:flex h-13 px-2 items-center justify-between shrink-0 select-none relative z-30 font-sans bg-transparent">
        {/* Left: Folder Name */}
        ...
        {/* Right Area: Search Bar + Refresh + Theme + Settings */}
        <div className="flex items-center gap-3 ml-auto">
          <div ref={desktopSearchRef} className="relative">
            <div
              className={`relative transition-all duration-300 ease-out ${
                isSearchFocused ? 'w-96' : 'w-60'
              }`}
            >
              <input
                ref={desktopInputRef}
            ...
            {isSearchFocused && (
              <div className="absolute right-0 top-full mt-2 w-96 bg-[var(--card-bg)] dark:bg-[#181a20] border border-[var(--card-border)] rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.18)] dark:shadow-[0_20px_50px_rgba(0,0,0,0.7)] z-50 overflow-hidden p-3 animate-toast apple-transition space-y-2.5 ring-1 ring-black/5 dark:ring-white/10">
                {displayContacts.length > 0 && (
